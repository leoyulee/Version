...
local Version: Version = {
    Version = "1.0.0";
    ClassName = "Version";
}--DataTypeClass:NewClass("Version","1.0.0",script)
...
function Version:__eq(value: Version): boolean
    ...
    assert(value.ClassName == "Version", "Attempt to compare Version with "..tostring(value.ClassName)..debug.traceback())
...
Version.new = function(MajorVersion,MinorVersion,PatchVersion): Version
    if type(MajorVersion) == "string" then
        return Version.fromString(MajorVersion)
    end
    local NewVersionType = setmetatable({
